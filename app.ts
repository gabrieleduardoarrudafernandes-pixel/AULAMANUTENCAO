...
class App {
  ...
  private bindEvents(): void {
    ...
    document.querySelectorAll<HTMLElement>(".slot-zone").forEach((el) => {
      ...
      el.addEventListener("drop", (e) => {
        ...
        if (partId) {
          this.selectPart(partId);
          if (partId === slotId) this.actionPart();
          else this.showWrongSlot(partId, slotId);
        }
      });
    });
  }

  private setMode(m: "assemble" | "disassemble"): void {
    this.mode = m;
  ...
  private selectPart(id: string): void {
    ...
    if (id === "ram") this.showRamBalloon();
    else this.hideRamBalloon();
  }

  /* ---- SHOW INFO ---- */
  private showInfo(id: string): void {
    const p = PARTS.find((x) => x.id === id)!;
  ...
  private renderTabela(tabela: TabelaRow[]): void {
  ...
  private updateActionBtn(): void {
    ...
    if (!this.selected) { btn.disabled = true; btn.textContent = "🔧 Instalar esta peca"; btn.className = "btn-action"; return; }

    if (this.mode === "assemble") {
      if (this.installed.has(this.selected)) {
        btn.disabled = true; btn.textContent = "✓ Ja instalada"; btn.className = "btn-action";
      } else {
        const expected = ASSEMBLY_ORDER[this.step];
        if (this.selected === expected) {
          btn.disabled = false; btn.textContent = "🔧 Instalar esta peca"; btn.className = "btn-action ready";
        } else {
          btn.disabled = false; btn.textContent = "⚠️ Ainda nao e a vez!"; btn.className = "btn-action wrong-order";
        }
      }
    } else {
      if (!this.installed.has(this.selected)) {
        btn.disabled = true; btn.textContent = "Nao instalada"; btn.className = "btn-action";
      } else {
        const expected = [...ASSEMBLY_ORDER].reverse()[this.step];
        if (this.selected === expected) {
          btn.disabled = false; btn.textContent = "🔩 Remover esta peca"; btn.className = "btn-action ready";
        } else {
          btn.disabled = false; btn.textContent = "⚠️ Remova as outras primeiro!"; btn.className = "btn-action wrong-order";
        }
      }
    }
  }
  ...
  /* ---- ACTION ---- */
  private actionPart(): void {
    if (!this.selected) return;
    if (this.mode === "assemble") this.assemble(this.selected);
    else this.disassemble(this.selected);
  }

  private assemble(id: string): void {
    const expected = ASSEMBLY_ORDER[this.step];
    if (id !== expected) {
      const p = PARTS.find((x) => x.id === id)!;
    ...
    if (id === "motherboard") {
      ...
      document.querySelectorAll<HTMLElement>(".slot-zone").forEach((s) => s.style.pointerEvents = "auto");
    }
  ...
  private disassemble(id: string): void {
    ...
    if (id === "motherboard") {
      ...
      // Disable inner slots
      document.querySelectorAll<HTMLElement>(".slot-zone").forEach((s) => s.style.pointerEvents = "none");
    }
  ...
  /* ---- WORKBENCH PLACEMENT ---- */
  private placeOnWorkbench(id: string): void {
    if (id === "motherboard") return;
    const wb = document.getElementById("workbench")!;
  ...
  private removeFromWorkbench(id: string): void {
    const el = document.getElementById(`placed-${id}`);
    if (el) { el.style.transition = ".3s"; el.style.opacity = "0"; el.style.transform = "scale(.4)"; setTimeout(() => el.remove(), 300); }
    const slot = document.getElementById(`slot-${id}`);
  ...
  /* ---- CONNECTION LINES ---- */
  private showConnectionLines(id: string): void {
    ...
    CONNECTIONS.forEach((c) => {
      if ((c.from === id && this.installed.has(c.to)) || (c.to === id && this.installed.has(c.from))) {
        if (document.getElementById(`conn-${c.from}-${c.to}`)) return;
  ...
  private removeConnectionLines(id: string): void {
    CONNECTIONS.forEach((c) => {
  ...
  private highlightSlot(id: string, on: boolean): void {
    const slot = document.getElementById(`slot-${id}`);
    if (slot && on) { slot.classList.add("flash"); setTimeout(() => slot.classList.remove("flash"), 500); }
  }
  ...
  private getInstallMsg(id: string): string {
  ...
  private getDisassembleMsg(id: string): string {
    const m: Record<string, string> = {
  ...
  private showWrongSlot(partId: string, slotId: string): void {
  ...
  /* ---- MODAL ---- */
  private showModal(emoji: string, title: string, msg: string, detail: string): void {
  ...
  private hideModal(): void { document.getElementById("modal-bg")!.classList.add("hidden"); }
  ...
  private hideCelebration(): void { document.getElementById("celebration-bg")!.classList.add("hidden"); }
}
